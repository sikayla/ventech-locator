...
try {
    ...
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); // Set error mode to exception for debugging
} catch (PDOException $e) {
    // If the connection fails, log the error and redirect
...
// Fetch client details (optional, can be used for displaying username, etc.)
try {
    ...
    if (!$user) {
        // If no user found, log the error and redirect
    ...
    header("Location: /ventech_locator/client_dashboard.php?error=fetch_user");
    exit;
}

// Fetch venues posted by the client
try {
...
try {
    ...
    $stmt->execute([$user_id]);
    $reservations = $stmt->fetchAll(PDO::FETCH_ASSOC); // Ensure correct array format
} catch (PDOException $e) {
    // Log the error and provide a fallback message
    error_log("Error fetching reservations for user $user_id: " . $e->getMessage());
    $reservations = [];  // Set empty array as fallback
}

// Check if the client has just added a venue and needs to fill out the details
$new_venue_message = "";
        ...
        <div class="flex-1 p-6">
            ...
            <?php if (!empty($new_venue_message)): ?>
                <div class="bg-yellow-100 p-4 rounded-md mb-8 text-center text-yellow-700">
                    <p><?= $new_venue_message ?> <a href="venue_details.php?id=<?= isset($venues[0]['id']) ? $venues[0]['id'] : '' ?>" class="text-blue-500">Click here to add details</a>.</p>
                </div>
            <?php endif; ?>
